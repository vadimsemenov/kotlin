...
fun recreateAndInitializeTypeParameters(
    ...
    val traitToFunTypeParameters: Map<TypeParameterDescriptor, TypeParameterDescriptorImpl> =
        recreateTypeParametersAndReturnMapping(originalParameters, newOwner)

    val typeParametersSubstitutor: TypeSubstitutor = createSubstitutorForTypeParameters(traitToFunTypeParameters)

    for ((traitTypeParameter, funTypeParameter) in traitToFunTypeParameters) {
        for (upperBound in traitTypeParameter.upperBounds) {
    ...
    val typeParameters: List<TypeParameterDescriptor> = ArrayList<TypeParameterDescriptor>(traitToFunTypeParameters.values)
    return SamConstructorTypeParameters(typeParameters, typeParametersSubstitutor)
}
...
fun recreateTypeParametersAndReturnMapping(
    ...
    val result = LinkedHashMap<TypeParameterDescriptor, TypeParameterDescriptorImpl>()
    for (typeParameter in originalParameters) {
        result[typeParameter] = TypeParameterDescriptorImpl.createForFurtherModification(
...
fun createSubstitutorForTypeParameters(
    ...
    val typeSubstitutionContext = HashMap<TypeConstructor, TypeProjection>()
    for ((key, value) in originalToAltTypeParameters) {
        typeSubstitutionContext[key.typeConstructor] =
            TypeProjectionImpl(value.defaultType)
    }
